...
def v2x_api_tests( suite, cpu_type = 'arm', total_frames = 10000, tx_pow = -10 ):
    ...
    test_links = [ tParam( tx = (0,0), rx = (1,0), proto_id = 0x1234, frames = 100, frame_rate_hz = 10, payload_len = 800, tx_power = tx_pow ), 
                    tParam( tx = (1,0), rx = (0,0), proto_id = 0x5678, frames = 100, frame_rate_hz = 10, payload_len = 800, tx_power = tx_pow ) ]
    ...
    test_links = [ tParam( tx = (0,1), rx = (1,1), proto_id = 0x1234, frames = 100, frame_rate_hz = 10, payload_len = 800, tx_power = tx_pow ), 
                    tParam( tx = (1,1), rx = (0,1), proto_id = 0x5678, frames = 100, frame_rate_hz = 10, payload_len = 800, tx_power = tx_pow ) ]
    suite.addTest(common.ParametrizedTestCase.parametrize(tc_link_api.TC_LINK_API, param = dict( params = test_links, target_cpu = cpu_type, verify_frames = frame_verification ) ) )
...
def nav_api_tests( suite, cpu_type = 'arm', sampling_time_sec = 60*10 ):
    ...
    suite.addTest(common.ParametrizedTestCase.parametrize(tc_navapi.TC_NAV_1,
                                                             param =  dict( uut_id = (0,0),  target_cpu = cpu_type,
...
def can_api_tests( suite, is_sc = True, cpu_type = 'arm' ):
    ...
    #test_param = dict( uut_id = (1,0))
    #suite.addTest(common.ParametrizedTestCase.parametrize(tc_can.TC_CAN_ERRONEOUS, param = test_param ))

    
    #test_param = dict( uut_id = (1,0), frames_rate = 500, frames_num = 10000, err_part = 0)
...
def parse_params():
    ...
    parser.add_argument( '-t', '--type', type=str, required=True, choices = ['sc', 'mc', 'all', 'sanity'], default='all', help='Testing Firmware type, either SC or MC or sanity on SC')
    ...
    #args = parser.parse_args( ['-v', 'sdk-4.6.0-beta2','-t', 'sc', ] )
    return args
...
if __name__ == "__main__":
    ...
    total_frames = 10 if args.quick else 10000
    sampling_time_sec = 120 if args.quick else 1200
    ...
    globals.setup.instruments.terminate()
