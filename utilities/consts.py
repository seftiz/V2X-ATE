...
def generate_const(name, max):
    for i in range(1, max):
        return [ (name, 1), (name, 2) ]


snmp = dict(
                    FREQ_SET = generate_const('wlanFrequency', 2),
                    TX_POWER = generate_const('wlanDefaultTxPower', 2),
                    DATA_RATE = generate_const('wlanDefaultTxDataRate', 2),
                    RX_MAC_COUNTER = generate_const('wlanFrameRxCnt', 2),
                    TX_MAC_COUNTER = generate_const('wlanFrameTxCnt', 2),
                    RX_RSSI = generate_const('wlanRssiLatestFrame', 2),
                    DCOC_STATUS = generate_const('wlanDcocStatus', 2),
                    DC_STATUS_DESCRPT = [ 'NotStarted', 'InProgress', 'Success', 'Failure' ],
                    RF_INDEX = generate_const('wlanRfindex', 2),
                    RF_ENABLED = generate_const('wlanRfEnabled', 2),
                    RF_FRONT_END_CONNECTED = generate_const('wlanRfFrontEndConnected', 2),
                    RF_FRONT_END_OFFSET = generate_const('wlanRfFrontEndOffset', 2),
                    TSSI_PINTERCEPT = generate_const('wlanTssiPintercept', 2),
                    TSSI_PSLOPE = generate_const('wlanTssiPslope', 2),
                    TSSI_INTERVAL = generate_const('wlanTssiInterval', 2),
                    RANDOM_BACKOFF_ENABLED = generate_const('wlanRandomBackoffEnabled', 2),
                    MAC_ADDRESS = generate_const('wlanMacAddress', 2),
                    ...
                    LOG_MODE = ['vcaLogMode'],
                    TX_PERIOD = generate_const('vcaTxPeriod', 2),
                    FRAME_LEN = generate_const('vcaFrameLen', 2),
                    TX_ENABLED = generate_const('vcaTxEnabled', 2),
...
common = dict(                    
                    ...
                    GPS_VERSION_TYPE = { 1:"TELIT SW Version: SL869 v3.1.5.1", 2:"sta8088-3.1.14-atk-1.0.0-rel", 2:"sta8088-3.1.14-atk-1.0.0-rel", 4:"sta8088-3.1.14-atk-1.0.0-rel", 5:"sta8088-3.1.14-atk-1.0.0-rel" },
