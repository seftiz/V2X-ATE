...
#define NO_SELECT_HACK
#define QUIT_SPECIAL_STATE "q"
...
#define _fprintf(sockfd, args...) \
  cli->_fprintf_len = snprintf(cli->_fprintf_buf, sizeof(cli->_fprintf_buf), args); \
  cli->_fprintf_buf[sizeof(cli->_fprintf_buf) - 1] = 0; \
  _send(cli, sockfd, cli->_fprintf_buf, cli->_fprintf_len);

#define CLI_PRINTF(sockfd, args...) \
  if(cli->protocol) {\
    if (cli->client)\
        _fprintf(sockfd,args);\
  }\
  else {\
    printf(args);\
  }\
...
// int cli_port_recv(struct cli_def *cli, int sockfd, void *buf, size_t len, int flags);

/**
 * Receives data from port.
 *
 * @param cli[in]    CLI interface.
 * @param sockfd[in] TCP socket.
 * @param buf[in]    buf to put the data.
 * @param len[in]    length of data to be received.
 * @param flags[in]  flags to the recv() function.
 *
 * @return length of reveived string.
 *
 * @note sockfd and flags used only in case telnet port receive.
 */
int cli_port_recv(struct cli_def *cli, int sockfd, void *buf, size_t len, int flags)
{
  int               rc    = -1;
  ...
  len = sizeof(sck_imq_msg_t);

  /* wait for data */
  /*
  while ( cq_is_empty( &myctx->cli_thread->buffer) ) {
    tx_thread_sleep(5);
  }
  */
  ...
  pos = (char*) buf;
  while ( len > 0 ) {
    cq_get( &myctx->cli_thread->buffer, pos );
    len--; pos++;
  }
  */
  while ( cq_is_empty( &myctx->cli_thread->buffer ) ) {
  ...
  while ( rc != 0 ) {
    rc = cq_get( &myctx->cli_thread->buffer, (char*) buf );
    // tx_thread_sleep(2);
  }
      
  /*
  rc =  imq_receive( myctx->imq_data_socket, (void*) &msg, &len, &atlk_wait_forever );
  if (atlk_error(rc)) {
    syslog( LOG_ERR, "imq_receive in ARC CLI failed: %s\n", atlk_rc_to_str(rc));
    return (-1);
  }
  */
 ...
 * @param cli[in]    CLI interface.
 * @param sockfd[in] TCP socket.
 * @param str[in]    data string to send.
 * @param len[in]    length of data to send.
 * @param flags[in]  flags to the send() function.
 *
 * @return length of reveived string.
 * @note sockfd and flags used only in case telnet port receive.
*/
int cli_port_send(struct cli_def *cli, int socket,const char* str, int len, int flags)
{
  ...
  while ( count > 0 ) {
    ...
    if ( count > MAX_BUFFER_SIZE ) {
      msg.len = MAX_BUFFER_SIZE;
    }
    else {
      msg.len = count;
    }
    
    memcpy( &msg.buffer, pos, msg.len );
    syslog( LOG_DEBUG, "CLI@%d, Tx %d: Total size :%d, MTU %d, Msg size %d data : %s", socket, ++i, len, MAX_BUFFER_SIZE, msg.len, msg.buffer);
      
    rc = imq_send( myctx->imq_data_socket, (void*) &msg, sizeof(sck_imq_msg_t), NULL);
    if (atlk_error(rc)) {
      syslog( LOG_ERR, "imq_send in ARC CLI failed: %s\n", atlk_rc_to_str(rc));
      printf("!!!! ERROR Failed to send data to IMQ\n" );
      return (-1);
    }
...
static ssize_t _send(struct cli_def *cli, int fd, const void *buf, size_t count)
{
    size_t written = 0;
    ssize_t thisTime =0;
    while (count != written)
    {
        thisTime = cli_port_send(cli, fd, (char*)buf + written, count - written, 0);
        if (thisTime == -1)
        {
            if (errno == EINTR)
                continue;
...
int cli_int_configure_terminal(struct cli_def *cli, const char *command, char *argv[], int argc)
{
    UNUSED(command);
    UNUSED(argv);
    UNUSED(argc);
...
static void cli_clear_line(struct cli_def *cli, int sockfd, char *cmd, int l, int cursor)
{
    int i;
    if (cursor < l)
    {
...
static int show_prompt(struct cli_def *cli, int sockfd)
{
    ...
    if (cli->state == STATE_SPECIAL) {
        return cli->special.prompt_cb(cli, sockfd);
    }

    if (cli->hostname)
        len += cli_port_send(cli, sockfd, cli->hostname, strlen(cli->hostname), 0 );

    if (cli->modestring)
        len += cli_port_send(cli, sockfd, cli->modestring, strlen(cli->modestring), 0 );

    return len + cli_port_send(cli, sockfd, cli->promptchar, strlen(cli->promptchar), 0 );
}
...
int cli_loop(struct cli_def *cli, int sockfd)
{
    ...
    cli_free_history(cli);
    if ( cli->protocol == CLI_PROTOCOL_TELNET )
    {
        static const char *negotiate =
            ...
            "\xFF\xFD\x03"
            "\xFF\xFD\x01";
        _send(cli, sockfd, negotiate, strlen(negotiate));
    }
    ...
    while (1)
    {
        ...
        while (1)
        {
            ...
            if (cli->showprompt)
            {
                ...
                {
                    case STATE_LOGIN:
                        _send(cli, sockfd, "Username: ", strlen("Username: "));
                        break;

                    case STATE_PASSWORD:
                        _send(cli, sockfd, "Password: ", strlen("Password: "));
                        break;
                    ...
                    case STATE_ENABLE:
                        ...
                        _send(cli, sockfd, cmd, l);
                        if (cursor < l)
                        {
                            int n = l - cursor;
                            while (n--)
                                 _send(cli, sockfd, "\b", 1);
                        }
                        break;
           ...
           if ((n = cli_port_recv(cli, sockfd, &c, 1, 0)) < 0)
            {
                if (errno == EINTR)
                    continue;
                    ...
                    while (back--)
                    {
                        ...
                        else
                        {
                            ...
                            if (cli->state != STATE_PASSWORD && cli->state != STATE_ENABLE_PASSWORD)
                            {
                                ...
                                _send(cli, sockfd, "\b", 1);
                                _send(cli, sockfd, cmd + cursor, strlen(cmd + cursor));
                                _send(cli, sockfd, " ", 1);
            ...
            if (c == CTRL('U'))
            {
                if (cli->state == STATE_PASSWORD || cli->state == STATE_ENABLE_PASSWORD)
                    ...
                    cli_clear_line(cli, sockfd, cmd, l, cursor);
            ...
            if (c == CTRL('I'))
            {
                ...
                else if (num_completions == 1)
                {
                    ...
                    cmd[l++] = ' ';
                    cursor = l;
                    _send(cli, sockfd, completions[0], strlen(completions[0]));
                    _send(cli, sockfd, " ", 1);
                }
                else if (lastchar == CTRL('I'))
                {
                    ...
                    {
                        _send(cli, sockfd, completions[i], strlen(completions[i]));
                        if (i % 4 == 3)
                            _send(cli, sockfd, "\r\n", 2);
                        else
                            _send(cli, sockfd, "     ", 1);
                    }
                    if (i % 4 != 3) _send(cli, sockfd, "\r\n", 2);
                        cli->showprompt = 1;
                }
                else
                {
            ...
            if (c == CTRL('E'))
            {
                ...
                {
                    if (cli->state != STATE_PASSWORD && cli->state != STATE_ENABLE_PASSWORD)
                        _send(cli, sockfd, &cmd[cursor], l - cursor);
            ...
            else
            {
                ...
                if (insertmode)
                {
                    ...
                    _send(cli, sockfd, &cmd[cursor], l - cursor + 1);
        ...
        else
        {
            ...
            if (cli->state == STATE_SPECIAL) {
              /* if we want to quit the special state */
              if (!memcmp(cmd, QUIT_SPECIAL_STATE, sizeof(QUIT_SPECIAL_STATE))) {
                cli->state = STATE_NORMAL;
              }
              else {
                cli->special.process_cb(cli, cmd);
              }
            } else {
...
int cli_match_filter_init(struct cli_def *cli, int argc, char **argv, struct cli_filter *filt)
{
    ...
    if (argc < 2)
    {
        CLI_PRINTF(cli->client_fd, "Match filter requires an argument\r\n");
    ...
    {
        CLI_PRINTF(cli->client_fd, "Invalid pattern \"%s\"\r\n", p);
...
int cli_match_filter(struct cli_def *cli, const char *string, void *data)
{
    ...
    if (!string) // clean up
    {
      if (state->flags & MATCH_REGEX) {
        ///* regfree*/regex_free(&state->match.re);
      } else
...
int cli_range_filter_init(struct cli_def *cli, int argc, char **argv, struct cli_filter *filt)
{
    ...
    if (!strncmp(argv[0], "bet", 3)) // between
    {
        if (argc < 3)
        {
            CLI_PRINTF(cli->client_fd, "Between filter requires 2 arguments\r\n");
    ...
    else // begin
    {
        if (argc < 2)
        {
            CLI_PRINTF(cli->client_fd, "Begin filter requires an argument\r\n");
...
{   
    ...
    if (argc > 1)
    {
       CLI_PRINTF(cli->client_fd, "Count filter does not take arguments\r\n");
