	...
	vector_iterate_entry (pos, &nfa->states, list) {
		vector_iterate_entry  (trans, &pos->table, list) {
			switch (trans->type) {
				case NFA_CHAR:
					if (trans->alpha.lo == trans->alpha.hi) {
						if (isprint (trans->alpha.lo))
							REGEX_PRINT ("\t\tn%" PRIu32 " -> n%" PRIu32 " [label = \"%c\"]\n",
										 pos->id, trans->next->id,
										 trans->alpha.lo);
						else
							REGEX_PRINT ("\t\tn%" PRIu32 " -> n%" PRIu32 " [label = \"%d\"]\n",
										 pos->id, trans->next->id,
										 trans->alpha.lo);
					}
					else {
						if (trans->alpha.lo >= 32 && trans->alpha.hi <= 126)
							REGEX_PRINT ("\t\tn%" PRIu32 " -> n%" PRIu32 " [label = \"%c-%c\"]\n",
										 pos->id, trans->next->id,
										 trans->alpha.lo, trans->alpha.hi);
						else
							REGEX_PRINT ("\t\tn%" PRIu32 " -> n%" PRIu32 " [label = \"%d-%d\"]\n",
										 pos->id, trans->next->id,
										 trans->alpha.lo, trans->alpha.hi);
					}
					break;
				...
				case NFA_FREE:
					REGEX_PRINT ("\t\tn%" PRIu32 " -> n%" PRIu32 " [label = \"FREE\"]\n",
								 pos->id, trans->next->id);
					break;
		...
		if (pos->final)
			REGEX_PRINT ("\t\tn%" PRIu32 " [shape=doublecircle]\n", pos->id);
	}
	REGEX_PRINT ("\t}\n");
}
...
dfa_print (struct dfa *dfa)
{
	...
	if (dfa->start->final)
		REGEX_PRINT ("\t\td%" PRIu32 " [shape=doublecircle];\n", dfa->start->id);

	vector_iterate_entry (pos, &dfa->states, list)
		for (i = 0; i < REG_MAGIC; i++)
			if (pos->table[i].next) {
				if (isprint (i))
					REGEX_PRINT ("\t\td%" PRIu32 " -> d%" PRIu32 " [label = \"%c\"];\n",
							pos->id, pos->table[i].next->id, i);
				else
					REGEX_PRINT ("\t\td%" PRIu32 " -> d%" PRIu32 " [label = \"%d\"];\n",
							pos->id, pos->table[i].next->id, i);
...
nfa_state_alloc (struct vector_node *head)
{
...
nfa_handle_alter (struct nfa *n1, struct nfa *n2)
{
	...
	n1->end = s;
	vector_movel_tail (n2->states.next, n2->states.prev, &n1->states);
	nfa_free (n2);
	return n1;
}
...
static inline int
nfa_handle_concat (struct vector_node *stack)
{
	struct nfa *cur, *next;
...
#define STACK_PEEK() vector_entry (stack.next, struct nfa, list)
...
nfa_compile_done:
	...
	nfa_handle_concat (&stack);
	return vector_entry(stack.next, struct nfa, list);
...
static unsigned int
__nfa_step (struct nfa_state *src, struct vector_node *head)
{
	struct nfa_state *pos;
	struct nfa_transition *trans;
	...
	vector_add_tail (&src->set, (struct vector_node *)&pos->set);
	length++;
...
#ifdef DEBUG
	REGEX_PRINT ("d%" PRIu32 ":", new->id);
	for (i = 0; i < nfa->size; i++)
		REGEX_PRINT (" n%" PRIu32 "%s", nfa->head[i]->id,
				(nfa->head[i]->final ? "(*)": ""));
	REGEX_PRINT ("\n");
#endif
             ...
             int eflags)
{
  ...
  (void) pmatch;
	...
	for (i = 0; i < string_len && pos; i++) {
		...
		if (!pos) {
//            REGEX_PRINT ("terminated at character %" PRIu32 " (%" PRIu32 ")\n", i, string[i]);
			return REG_NOMATCH;
		}
		else if (pos->final) {
//            REGEX_PRINT ("matched at character %" PRIu32 " (%" PRIu32 ")\n", i, string[i]);
			return REG_SUCCESS;
		}
	}
