...
int cmd_send_wsmp(struct cli_def *cli, const char *command, char *argv[], int argc)
{
    int i;
    ...
    if ( strcmp(argv[0], "?") == 0 ) {
        cli_print(cli, "[-frames 1-100K] [-rate 12] [-user_priority {0-7}] [-data_rate 54,54,45] [-interface_idx 0|1] [power_dbm8 0-30]\n \
                       [-psid 1234] [elem_id 1-23] [-tx_power_used 1-80] [-data_rate 1]" );
...
int main()
{
    ...
    cli = cli_init();
    cli_set_banner(cli, "ATE - Auto-talks test environment ");
    cli_set_hostname(cli, "ate");
    ...
    cli_register_command(cli, c, "regular", cmd_show_regular, PRIVILEGE_UNPRIVILEGED, MODE_EXEC, "Show the how many times cli_regular has run");
    ...
    c = cli_register_command(cli, NULL, "send_frame", NULL, PRIVILEGE_UNPRIVILEGED, MODE_EXEC, "send message frame via SDK");
    cli_register_command(cli, c, "wsmp", NULL, PRIVILEGE_UNPRIVILEGED, MODE_EXEC, "send WSMP frame");
    cli_register_command(cli, c, "cam", cmd_test, PRIVILEGE_UNPRIVILEGED, MODE_EXEC, "Show the counters that the system uses");
    ...
    cli_register_command(cli, c, "regular", cmd_debug_regular, PRIVILEGE_UNPRIVILEGED, MODE_EXEC, "Enable cli_regular() callback debugging");
