...
class Link(object):
    ...
    def service_create(self, type = "hw", server_ip = None ):
        ...
        # prevet initlize service twice on same uut
    ...
    def socket_create(self, if_idx, frame_type, proto_id):
        ...
        cmd += "-if_idx %d -frame_type %s -proto_id 0x%x" % (if_idx, frame_type, proto_id)
        data = self._if.send_command(cmd)
    ...
    def trasnmit(self, payload_len = None, tx_data = None, dest_addr = None, frames = 1, rate_hz = 1, user_priority = None, data_rate = None, power_dbm8 = None):
    ...
    def set_socket_addr( self, address ):
...
class Can(Link):
    ...
    def __init__(self, interface):
        super(Link, self).__init__(interface)
        self._name = "can"

    def socket_create(self, can_filter):
        """"can socket create -filter_count 0|N [can_id_1 0x1 can_id_mask_1 0xFFFFFFFFU .... can_id_N 0x1 can_id_mask_N 0xFFFFFFFFU]"""
        ...
        for i, can in enumerate(filter_can):
            cmd += "-can_id_%d 0x%x -can_id_mask_%d 0x%x" % (i, can, i, filter_can[can] )
    ...
    def trasnmit(self, frames = 1, rate_hz = 1, can_id = None, can_data = None, data_size = None):
        ...
        cmd += (" -can_id %s"  % can_id) if not can_id is None else ""
        if not can_data is None:
            cmd += (" -can_data %s"  % can_data)
        elif not data_size is None:
            cmd += (" -data_size %d"  % data_size)

        self._if.send_command(cmd)

    def receive(self, frames, timeout = None, print_frame = None):
        """ can socket rx [-frames 1- ... [-print (0|1)]" """
        cmd = "%s socket rx -frames %s" % frames
        cmd += (" -print %d"  % print_frame) if not print_frame is None else ""
        # cmd += (" -timeout_ms %d"  % timeout) if not timeout is None else ""
        self._if.send_command(cmd)
...
class V2xCli(object):
    ...
    def __init__(self, version = ""):
        ...
        # declare modules
        self.link = Link( self._if)
        self.can = Can( self._if)
        self.nav = Nav( self._if )
...
class V2xCliDebug(V2xCli):
    ...
    def __init__(self, version = ""):
        #return super(V2xCli, self).__init__(methodName, param)
        V2xCli.__init__(self, "")
