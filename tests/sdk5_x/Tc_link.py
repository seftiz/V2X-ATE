...
class TC_LINK(common.V2X_SDKBaseTest):
    ...
    def unit_configuration(self):
        ...
        self._rc = self.v2x_cli.link.device_register("00:02:cc:f0:00:07",0,"eth1")
        self._rc = self.v2x_cli.link.service_register("v2x",0)
        self._rc = self.v2x_cli.link.socket_create(self.if_index - 1, "data", 1234 )
        self._rc = self.v2x_cli.link.socket_tx(1000, 10 )
    ...
    def start_dut_sniffer(self, cli_interface, idx, type):
        ...
        self.sniffer_file.append(os.path.join( common.SNIFFER_DRIVE ,  self._testMethodName + "_" + "dut" + idx + "_" + type + "_" + time.strftime("%Y%m%d-%H%M%S") + "." + 'pcap'))  
        time.sleep(2)
        ...
        time.sleep(1)
        self.dut_embd_sniffer.start( if_idx = rf_if, server_ip = None, server_port = sniffer_port, sniffer_type = type)
