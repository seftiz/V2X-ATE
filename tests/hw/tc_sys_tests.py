    ...
    def test_sensitivity(self):
        ...
        for i in range(channel_loop):
            ...
            with open(sens_fname, "a+") as out_file:
                hw_tools.print_and_log(out_file, "{:s}, {:s}, {:s}, {:s}, {:s}, {:s}, {:s},".format(time.strftime("%d/%m/%Y"),str(c.common['dsrc_channel_models_list'][i]),str(self.channel_name),str(rate),str(ch),sens_point,status_s))
            out_file.close()
        ...
        try:
            sensitivity_res_loader_ = np.loadtxt(sens_fname, delimiter=',', skiprows=1, usecols=(2,4))
            ...
            with open(sens_fname, "a+") as out_file:
                hw_tools.print_and_log(out_file, "%s, Maximum sensitivity %s, rate %s,"% (str(self.channel_name),str(rate),str(max_sens_by_rate)))
                hw_tools.print_and_log(out_file, "%s, Minimum sensitivity %s, rate %s,"% (str(self.channel_name),str(rate),str(min_sens_by_rate)))
            out_file.close()
        except:
