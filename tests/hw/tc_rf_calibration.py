    ...
    def test_tx_power_calibration(self):
        ...
        for iter_power in ch_power:
            ...
            if (fifo_meas_status == False):
                SUM = 0
                SUMresTxEVM = 0
                SAMPLES = 5 #after addition delta
                resTxEVM = 0
                SUMdetector_meas = 0
                TesterDevice.vsa_settings(Freq, c.common['VSA_MAX_SIGNAL_LEVEL'], Port, Atten, c.common['VSA_TRIGGER_LEVEL'], c.common['VSA_CAPTURE_WINDOW'] )
                time.sleep(0.3)
                ...
                for i in range (0,SAMPLES):          
                    ...
                    SUMdetector_meas += int(regs.get(("mac0",c.register['TSSI_FIFO'][self.param['macIf']]))&hw_tools.bit_mask(10))

                # Collect the measurements                
                tx_power_list.append(SUM/SAMPLES)
                detector_meas_list.append(SUMdetector_meas/SAMPLES)
...
if __name__ == '__main__':
    ...
    if (8 in choice_num) or (9 in choice_num):
        ...
        rep_details = [N,c.common['type_dict'].get(type),sw_version, serial_number, c.common['gps_version_dict'].get(type), str(localtime),'25C', evk_ip, str(ethaddr)]
