 ...
 *   - LP_GetResultSpectralMask();
 *   - LP_GetResultSpectralMask_HT40();
 *   - LP_GetCapture();
 *   - LP_GetSampleData();
 ...
 *   - LP_LoResultSpectralFlatness11AG();
 *   - LP_LoResultSpectralFlatness11N();
...
//! Parameter structure for 11n/ac settings.
typedef struct _11NACParamTag
{
    //! PSDU reference file (needed for Composite EVM).
	char *pcReferenceFile;
	...
	//! Channel Estimate on full packet.
	/*!
	 *         - 0: Disable (Default)
	 *         - 1: Enable
	 */
	int iEnableFullPacketChannelEst;

	//! Frequency Correction options.
	/*!
...
//! Parameter structure for 11a/g settings.
typedef struct _11AG_ParamTag
{
	//! Phase Correction Mode with the following valid options.
	/*!
	...
	//! Channel Estimate with the following options.
	/*!
	...
	//! Symbol Timing Correction with the following options.
	/*!
	...
	//! Frequency Sync. Mode with the following options.
	/*!
	...
	//! Amplitude Tracking with the following options.
	/*!
...
//! Parameter structure for 11b settings.
typedef struct _11B_ParamTag
{
	//! Number of equalizer taps, valid options are as follows.
	/*!
	...
	//! DC removal, valid options are as follows.
	/*!
	 *         - IQV_DC_REMOVAL_OFF: DC Removal OFF (Default).
	 *         - IQV_DC_REMOVAL_ON: DC Removal ON.
	 */
	int iDCremove11b_flag;

	
    //! 802.11b method, valid options are as follows.
	/*!
...
//! Parameter structure for Tx setting.
typedef struct TxCommonInfoTag
{
	//! It can be VHT, HT_MF, HT_GF, or NON_HT.
	int iPacketFormat;

	//! It can be BW_20MHZ, BW_40MHZ, BW_80MHZ, BW_160MHZ,or BW_80_80MHZ.
	int iBandwidth;
	
	//! It can be OFDM or DSSS.
	int iModulation;

	//! In mps sequencial captures, you can assign the packet number you want to skip in the beginning of the capture.
	int	iSkipPktCnt;

	//! Which port you used.
	/*!
	 * Valid values are IQV_PORT_LEFT (RF1) and IQV_PORT_RIGHT(RF2).
	 */
	int iRfPort;

	//! The tested center frequency in MHz.
	double dFreqMHz;

	//! The trigger level in VSA.
	double dTriggerLeveldBm;

	//! The reference level in VSA.
	double dRefLeveldBm;

}TxCommonInfo;

//! Parameter structure for Rx setting.
typedef struct RxCommonInfoTag
{
	//! A wave form name, specifying which wave form should be used.
	char *pcWaveformName;

	//! Number of packets to be transmitted for power level.
	/*! 
	 * Maximum number of packets is 32767.
	 */
	int iPacketCount;

	//! The RF ports used for RX PER testing.
	/*! 
	 * Valid values are IQV_PORT_LEFT (RF1) and IQV_PORT_RIGHT(RF2).
	 */
	int iRfPort;
	
	//! Frequency (MHz) on which DUT operates.
	double dFreqMHz;

	//! The start of power level (dBm at IQXEL RF ports).
	double dPowerLevelStartdBm;

	//! The stop of power level (dBm at IQXEL RF ports).
	double dPowerLevelStopdBm;

	//! Power level step(dB).
	/*!
	 * Minimum step is 0.5dB.
	 */
	double dStepdB;

	//! Enable ack in rx or not. 
	BOOL   bAckEnabled;

}RxCommonInfo;


typedef struct ExecParamDefTag
{
	double dFreqMHz;
	double dVsgTriggerLevel;
	int    iChangeDirectionTime;
	int    iTxCaptureTime;
	std::string sMode;
}ExecParamDef;
...
enum TEST_MODE
{
	...
	TX,
	TX_VHT80_MASK,
	FIXED_PACKET, //For fixed ack, we also use the same sequnce as fixed packet's and just replace parameter by using $.
	TEST_MODE_NUMBER
};
 ...
 *				- =1: IQxel160 2x2 80Mhz MIMO mode;if ipAddress1 and ipAddress2 both must be the same (IQxel160's IP address)
 *                    IQxel160 160M MIMO mode;if ipAddress1 and ipAddress2 are different(two IQxel160s' IP addresses)
...
//! Get spectral mask result for HT40
/*!
* \param[out] ViolationPercent The Percent of violations where the mask is below the PSD data value.
* \param[out] length Array length of the measured result.
* \param[out] freqs_MHz Array of the measured frequency.
...
//! Get Lo Leakage and Spectral Flatness for 11a/g
/*!
* \param[out] carrierNo The number of sub carrier which is closet to specification.
* \param[out] margin The margin value of the closet sub carrier for specification.
* \param[out] loLeakage Transmitter center frequency leakage
* \param[out] flatnessPass Flatness test result: true=PASS, false=FAIL
* \param[out] sub_carrier The numbers of sub carriers which are closet to specification in each section
* \param[out] data_per_carrier The margin values of the closet sub carrier in each section for specification
* \param[out] average_center_power The average power of upper-1 section and lower-1 section
* \return ERR_OK.
*/
...
//! Get Lo Leakage and Spectral Flatness for 11n
/*!
* \param[in] wifiMode One enum value of WIFI_MASK_TYPE.
* \param[out] carrierNo The number of sub carrier which is closet to specification.
* \param[out] margin The margin value of the closet sub carrier for specification.
* \param[out] loLeakage Transmitter center frequency leakage
* \param[out] flatnessPass Flatness test result: true=PASS, false=FAIL
...
//! Prepare to create a new sequence.
/*!
* \param[in] iTestType  TX or RX
* \param[in] iSequenceNum  Assign a sequence with a index number.
*/
IQMEASURE_API int		LP_IQXELEXT_NewSequence(int iTestType, int *piSequenceNum);


//! Finish combination of SCPI commands into a sequence command.
/*!
*/
IQMEASURE_API int		LP_IQXELEXT_FinishSequence();



//! Execute the sequence.
/*!
* \param[in] iSequenceNum  Execute a specific sequence
*/
IQMEASURE_API int		LP_IQXELEXT_Execute(int iSequenceNum, ExecParamDef ExecParam);

//! Add multiple capture type with specified capture length and count.
/*!
 * \param[in] iAnalysisType  analysis type. With this parameter, we can know which tests need to be analyzed by referring to ANALYSIS_TYPE.  
 * \param[in] iCaptureLengthUs Capture length in us.                                         Max allowed: 1023 us
 * \param[in] iCaptureCnt Capture count.                                                     Max allowed: 255
*/
IQMEASURE_API int		LP_IQXELEXT_AddMultiCalc(int iAnalysisType, int iCaptureLengthUs, int iCaptureCnt);
														
														
																												
//! IQXEL ACK-based RX PER Test
/*!
 * \param[in] iMode There are two modes, fixed ack or fixed packet.
 * \param[in] pcWaveformName A wave form name, specifying which wave form should be used
 * \param[in] dFreqMHz Frequency (MHz) on which DUT operates
 * \param[in] dPowerLevelStartdBm The start of power level (dBm at IQXEL RF ports)
 * \param[in] dPowerLevelStopdBm The stop of power level (dBm at IQXEL RF ports)
 * \param[in] dStepdB Power level step(dB).  Minimum step is 0.5dB.
 * \param[in] iPacketCount Number of packets to be transmitted for power level. Maximum number of packets is 32767.
 * \param[in] iRfPort The RF ports used for RX PER testing. Valid values are IQV_PORT_LEFT (RF1) and IQV_PORT_RIGHT(RF2).
 *
 * \return ERR_OK if the RX PER is successful; otherwise, call IQXELEXT_GetLastErr() for detailed error message.
 *
 * \remark 
 *         -# In most cases, the stop power level would be lower than the start power level, but it could be
 *            higher than the start power level.  In either way, step would be relative, meaning always positive.
...
IQMEASURE_API int		LP_IQXELEXT_AddFastRxPer(RxCommonInfo RxInfo);
	

//! VSA Multi-packet analysis capture. 
/*!
 * \param[in] iTimeoutInMs  - Set timeout to wait for tester arm ready.
 * \return ERR_OK if the VSA capture is done successful; otherwise, call IQXELEXT_GetLastErr() for detailed error message.
*/
IQMEASURE_API int		LP_IQXELEXT_InitiateMultiAnalysis(int iTimeoutInMs = 2000);
...
/*!
 * \param[in] iTimeoutInMs  - Set timeout to wait for capture done.
 * \return ERR_OK if the api succeeded; otherwise call LP_GetErrorString() for detailed error message.
 */
IQMEASURE_API int       LP_IQXELEXT_WaitForCaptureDone(int iTimeoutInMs = 2000);

//! Set measurement parameters for the analysis in 802.11 n and 802.11ac.
/*!
 * \param[in] TxInfo Parameter structure \ref TxCommonInfoTag for Tx setting
 * \param[in] Param Parameter structure \ref _11NACParamTag for 802.11n/ac setting
 *
 * \return ERR_OK if the api succeeded; otherwise call LP_GetErrorString() for detailed error message.
 */
IQMEASURE_API int       LP_IQXELEXT_SetMeasurementParam_11NAC(TxCommonInfo TxInfo, _11NACParam Param); 						 

//! Set measurement parameters for the analysis in 802.11a/g.
/*!
 * \param[in] TxInfo Parameter structure \ref TxCommonInfoTag for Tx setting
 * \param[in] Param Parameter structure \ref _11AG_ParamTag for 802.11a/g setting
 *
 * \return ERR_OK if the api succeeded; otherwise call LP_GetErrorString() for detailed error message.
 */
IQMEASURE_API int		LP_IQXELEXT_SetMeasurementParam_11AG(TxCommonInfo TxInfo, _11AG_Param Param);

//! Set measurement parameters for the analysis in 802.11b.
/*!
 * \param[in] TxInfo Parameter structure \ref TxCommonInfoTag for Tx setting
 * \param[in] Param Parameter structure \ref _11B_ParamTag for 802.11b setting
 *
 * \return ERR_OK if the api succeeded; otherwise call LP_GetErrorString() for detailed error message.
 */
IQMEASURE_API int		LP_IQXELEXT_SetMeasurementParam_11B(TxCommonInfo TxInfo, _11B_ParamTag Param);
...
//! Get the number of results for a particular measurement element (pointed to by indexResult) by measurement name
/*!
 * \param [in] pcMeasurementName         measurement name
 * \param [in] iIndexResult             index to a particular measurement element
 * \param [out] piNumberOfMeasurementElement return number of measurement element 
 * \return ERR_OK if successful; otherwise, call IQXELEXT_GetLastErr() for detailed error message.
*/
IQMEASURE_API int		LP_IQXELEXT_GetNumberOfMeasurementElements(char *pcMeasurementName, int iIndexResult, int *piNumberOfMeasurementElement);

//! Get the number of results for a particular measurement element (pointed to by indexResult) by measurement name
/*!
 * \param [in] pcMeasurementName         measurement name
 * \param [out] piNumberOfPerPowerLevel return number of per power level 
 * \return ERR_OK if successful; otherwise, call IQXELEXT_GetLastErr() for detailed error message.
*/
IQMEASURE_API int		LP_IQXELEXT_GetNumberOfPerPowerLevels(char *pcMeasurementName, int *piNumberOfPerPowerLevel);

//! Retrieve Analysis Results [Double] in average, minimum and maximum value
/*!
 * \param[in]  pcMeasurementName          The measurement name.  Please refer to \ref group_scalar_measurement "Vector Measurements" for all available measurement names.
 * \param[out] pdAverage Average value    Average value for all the result(s). Result set can be more than one if the specified capture count is larger than one when using multi-capture function 'IQXELEXT_VsaMultiCapture(...)'
 * \param[out] pdMinimum Minimum value    Minimum value of all the result(s)
 * \param[out] pdMaximum Maximum value    Maximum value of all the result(s)
 *
 * \return ERR_OK if successful; otherwise, call IQXELEXT_GetLastErr() for detailed error message.
 */
IQMEASURE_API int		LP_IQXELEXT_GetDoubleMeasurements(char *pcMeasurementName, double *pdAverage, double *pdMinimum, double *pdMaximum);


//! Retrieve Analysis Results [char] 
/*!
 * \param[in] pcMeasurementName The measurement name.  Please refer to \ref group_scalar_measurement "Vector Measurements" for all available measurement names.
 * \param[out] pcResult Result in 'char' pointer type
 * \param[in] iBufferSize Indicates the buffer size of result
 * \param[in] iIndexResult (starts with 0 as the first set of result) indicates which set of result to be retrieved. It has to be less than the number of capture set in IQXELEXT_VsaMultiCapture(...)
 *
 * \return ERR_OK if successful; otherwise, call IQXELEXT_GetLastErr() for detailed error message.
 */
IQMEASURE_API int		LP_IQXELEXT_GetStringMeasurement(char *pcMeasurementName, char *pcResult, int  iBufferSize, int iIndexResult = 0);

//! Retrieve Analysis Results [Double] in vector format ( more than one value in one particular result set as indicated by indexResult )
/*!
 * \param[in] pcMeasurementName The measurement name.  Please refer to \ref group_vector_measurement "Vector Measurements" for all available measurement names
 * \param[out] pdValues Returns the real part of vector
 * \param[in] iBufferLength Indicates the length of bufferReal and bufferImag (in number of element)
 * \param[out] *piDataSize indicates the number of element in the result. If the dataSize is greater than bufferLength, then only the data with bufferLength will be filled up in the buffer
 * \param[in] iIndexResult (starts with 0 as the first set of result) indicates which set of result to be retrieved. It has to be less than the number of capture set in IQXELEXT_VsaMultiCapture(...)
 *
 * \return ERR_OK if successful; otherwise, call IQXELEXT_GetLastErr() for detailed error message.
 */
IQMEASURE_API int		LP_IQXELEXT_GetVectorMeasurement(int iStandard, char *pcMeasurementName, double pdValues[], int *piDataSize, int iIndexResult = 0);

//! Retrieve Analysis Results [int] 
/*!
 * \param[in] pcMeasurementName The measurement name.  Please refer to \ref group_vector_measurement "Vector Measurements" for all available measurement names
 * \param[out] piResult Result in 'int' type
 * \param[in] iIndexResult (starts with 0 as the first set of result) indicates which set of result to be retrieved. It has to be less than the number of capture set in IQXELEXT_VsaMultiCapture(...)
*/
IQMEASURE_API int		LP_IQXELEXT_GetIntMeasurement(char *pcMeasurementName, int *piResult, int iIndexResult = 0);


//! Pre-load wave form files.
/*!
 * \param[in] pcWaveformName The waveform file name.
 * \param[in] iLoadInternalWaveform Choose whether to load modFile from internal tester (1) or upload from file (0) default is (0).
 *            This option is only valid with IQxel tester other wise it is ignored.
*/
IQMEASURE_API  int       LP_IQXELEXT_LoadWaveformFile(char *pcWaveformName, int iLoadInternalWaveform = 0);




//! Fast per tests in IQxel temporarily.
/*!
 * \param[in] uiPort  The rf port used
 * \param[in] iFreqMHz  The testd frequency in MHz
 * \param[in] uiVsgTransmittPtkCnt  The transmitted packet count in VSG
 * \param[in] dChangeDirectionTime  The length of waveform add a tolerance, 20us.
 * \param[in] dVsgTrigPower  The triggered power in VSG
 * \param[in] pcMpsMode  "FPC" means fixed packet and "FAC" means fixed ack
 * \param[in] pcWaveFileName  The waveform files played
 * \param[out] puiAckNumReturn  The returned ack number
*/
IQMEASURE_API  int       LP_IQXEL_Fast_Per(unsigned int uiPort, int iFreqMHz, unsigned int uiVsgTransmittPtkCnt, double dChangeDirectionTime, 
										  double dVsgTrigPower, char *pcMpsMode, char *pcWaveFileName, unsigned int *puiAckNumReturn);
