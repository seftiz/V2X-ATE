...
class IQ2010_VSA(IQ2010_Instrument):
    ...
    def get_spectral_mask_measure (self, bandwidth_index = 0):
        ...
        */
        ...
        """
        # def arr_ptr_2_list( ptr , len ):
            # array = (ctypes.c_double * len)()
            # ctypes.memmove(array, ctypes.byref(ptr), len)
            # temp = []
            # for i in range(len):
                # temp.append( array[i] )
        ...
        violation_percent = ctypes.c_double()
        length_array = ctypes.c_int()
        freqs_mhz_array = ctypes.c_double()
        trace_dbm_array = ctypes.c_double()
        mask_array = ctypes.c_double()
        bw_idx = ctypes.c_int(bandwidth_index)
        ...
        print "call specral measure"

        #rc = spectral_mask_measure(bw_idx, violation_percent, length_array, ctypes.byref(freqs_mhz_array), ctypes.byref(trace_dbm_array), ctypes.byref(mask_array) )
        try:
            rc = spectral_mask_measure(bw_idx, violation_percent, length_array, freqs_mhz_array, trace_dbm_array, mask_array )
            if rc != 0:
                raise Exception( rc, self.get_error_string(rc) )  
        ...
        print "ok"    
        #cast(freqs_mhz_array, POINTER(c_double))
        ...
        # array = (ctypes.c_double * length_array.value)()
        # ctypes.memmove(array, ctypes.byref(freqs_mhz_array), length_array.value)
        # temp = []
        # for i in range(length_array.value):
            # temp.append( array[i] )
            
        # results['freqs_mhz_array'] = temp
        
        
        #print arr_ptr_2_list( freqs_mhz_array, length_array.value)
            
        #results['freqs_mhz_array'] = arr_ptr_2_list( freqs_mhz_array, length_array.value)
        #results['trace_dbm_array'] = arr_ptr_2_list( trace_dbm_array, length_array.value)
        #results['mask_array'] = arr_ptr_2_list ( mask_array, length_array.value)
        
        #print "Here is the results {}".format ( results )
        # return results

        
    def get_spectral_mask_fft():
        """
        ...
        result2 = LP_GetScalarMeasurement("length");
        printf("Number of point on spectrum: %.0f\n", result2);

        for(int i = 0; i< 8; i++)
        {
              result3 = LP_GetScalarMeasurement("spectrumMarginOffsetFreqHz",i);
              fprintf(FFT,"%.2e,", result3);
        }
        fprintf(FFT,"\n\n\n\n");

        for(int i = 0; i< 8; i++)
        {
              result3 = LP_GetScalarMeasurement("spectrumCheck",i);
              fprintf(FFT,"%.0f,", result3);
        }
        fprintf(FFT,"\n\n\n\n");

        for(int i = 0; i< 8; i++)
        {
              result3 = LP_GetScalarMeasurement("spectrumMarginDb",i);
              fprintf(FFT,"%.2e,", result3);
        }
        fprintf(FFT,"\n\n\n\n");

        result3 = LP_GetScalarMeasurement("spectrumAverViolationPercentage");
        printf("spectrumAverViolationPercentage: %.3f\n", result3);

        result3 = LP_GetScalarMeasurement("spectrumAverObw");
         ...
         */
        """ 
    def get_vector_measurement(measurement, buffer_real, buffer_imag, buffer_length):
        """
         ...
         */    
        """
        vector_meas = self.hwd.LP_GetVectorMeasurement
        vector_meas.restype = ctypes.c_double
        vector_meas.argtypes = [ ctypes.c_char_p, ctypes.c_double, ctypes.c_double, ctypes.c_int ]
        
        buffer_char = vector_meas(measurement, buffer_real, buffer_imag, buffer_length)
